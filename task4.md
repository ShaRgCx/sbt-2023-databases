...
Основным отличием Прометея от конкурентов является модель pull данных, когда СУБД сама собирает данные с указанных в конфиге портов вместо того, чтобы ждать, когда отслеживаемые машины будут отсылать данные. Для этого необходимо настроить отслеживаемые машины так, чтобы они по определенному порту передавали необходимые метрики в понятном Prometheus формате. 

Кроме того, в Прометее поддерживается система оповещений, которая в случае нарушения какой-либо метрики при мониторнке будет сообщать нам на почту/смс/звонком о том, что у нас что-то сломалось в системе и нужно срочно бежать домой и чинить произошедшее.
...
В СУБД используется рукописный database engine Prometheus.
...
Кластеризация не поддерживается. (Поддерживается в расширении Prometheus Operator, но это другой уровень абстракции)
...
Прометей сам автоматически создает индексы внутри себя для удобства хранения. Существует два типа индексов:
+ The postings index -- Хранит связь между лейблами и сетами, в которых хранятся эти лейблы
+ The series index -- Показывает, какой из блоков принажлежит к какой серии.

## 8.Как строится процесс выполнения запросов в вашей СУБД?
Выполнение запроса разбивается на 4 основных шага:
+ 1. Определение, к какому блоку принадлежит time range запроса. На этом шаге Прометей отсекает все остальные блоки, которые не были указаны в запросе.
+ 2. Поиск серии, соответствующей выбраному лейблу, использую posting index. На этом этапе каждый оставшийся блок сканируется на серию, которая соответствует выбранному лейблу. После этого у нас остается набор серий, которые соответствуют каждому из указанных в запросе лейблов.
+ 3. Аналогично второму шагу, но теперь series index. Прометей ищет чанки, которые соответствуют сериям, найденным на прошлом этапе.
+ 4. Достаем данные из получаенных чанком и применяем все операции, указанные в запросе.

## 9.Есть ли для вашей СУБД понятие «план запросов»? Если да, объясните, как работает данный этап.
Понятие "план запроса" в СУБД отсутствует.
...
Методы восстановления не поддерживаются, данные не реплицируются на другие ноды и не кластеризуются (можно, но в Prometheus Operator и осторожно). Но при этом всегда сохраняется текущий блок данных, защищенный от падений через WAL, поэтому в случае падения сервера, всегда будет доступен последний блок данных при рестарте.
...
В самом Прометее шардинг отсутствует, но его можно настроить в Prometheus Operator. (Опять же, это более высокоуровневая штука, но вот [тут](https://github.com/prometheus-operator/prometheus-operator/blob/main/Documentation/user-guides/shards-and-replicas.md) есть гайд по настройке шардинга. В данном случае используется Instance Sharding. Кроме того, шардинг поддерживается в Grafana, которая нативно совместима с Прометеем, там у нас есть выбор из 3х типов Instance sharing, Hashmode sharding (сегментирование происходит с помощью пары правил hashmod/keep relabel) и Host Filtering (здесь демонсетом запускаем экземпляр на каждой машине, и только на этой машине осуществляется сбор таргетов))
...
Основные методы защиты, используемые Прометеем указаны на [этой странице](https://prometheus.io/docs/operating/security/). В будущем планируется использование TLS, но пока это только в планах.
...
+ http://onreader.mdl.ru/PrometheusUpAndRunning/content/Ch01.html
